...
class AddClientDataController extends Controller
{
    public function store(Request $request)
    {
        if (Auth::user()->role == 'Shopkeeper') {
            ...
            if($product->name_field == 1){//producto a cambiar
                $productRequirements = $productRequirements.$request->input('nameField').'\r\n';
            }
            if($product->account_type == 1){
                $productRequirements = $productRequirements.$request->input('accountType').'\r\n';
            }
            if($product->account_number == 1){
                $productRequirements = $productRequirements.$request->input('accountNumber').'\r\n';
            }
            if($product->email == 1){
            ...
            if($product->phone_number == 1){
                $productRequirements = $productRequirements.$request->input('phoneNumber').'\r\n';
            }
            if($product->code == 1){
